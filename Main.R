...
# similarly, function to calculate ESV for each service of each land use
# bug: need improve
GetSumESVLanduse <- function(x, es) {
  outline <- subset(land.use, land_use == x)
  mask(esv.ebk[[es]], outline) %>% 
    global(fun = "sum", na.rm = TRUE) %>% 
    .[1, 1] %>% 
    return()
}
...
kES <- data.frame(
  ...
  es = c("carbon_stor", "carbon_seq", 
         "no2_rem", "o3_rem", "pm25_rem", "so2_rem", "avo_ro"), 
...
quadrat <- 
  ...
  st_transform(6668)
names(quadrat)[1] <- "qua_id"

# ecosystem services interpolation by EBK
ebk <- vector("list", 7)
names(ebk) <- kES$es_ebk
for (i in kES$es_ebk) {
...
qua.data <- indv.data %>% 
  ...
  summarise(
    ...
    lai_mid = median(lai), 
    # sum of ES
    across(all_of(c(kES$es, kES$esv)), sum)
  ) %>% 
  ungroup() %>% 
...
names(sum.landuse) <- c("landuse", kES$es)

# calculate total ESV of each land use
sum.esv.landuse <- data.frame(land_use = kLanduse)
for (j in kES$es_ebk) {
  es.sum <- c()
  for (i in kLanduse) {
    es.sum <- c(es.sum, GetSumESVLanduse(i, j))
  }
  sum.esv.landuse[[j]] <- es.sum
}
names(sum.esv.landuse) <- c("landuse", kES$esv)
...
lu.data <- indv.data %>% 
  ...
  left_join(sum.landuse, by = "landuse") %>% 
  # 加入ESV
  left_join(sum.esv.landuse, by = "landuse") %>% 
  # 重排土地利用顺序
...
val.res <- 
  ...
  do.call(
    rbind, 
    list(
      lapply(kES$es_ebk, ReadDbf, intrpl = "EBK") %>% 
        do.call(rbind, .), 
      lapply(kES$es_ebk, ReadDbf, intrpl = "IDW") %>% 
        do.call(rbind, .), 
      lapply(kES$es_ebk, ReadDbf, intrpl = "OK") %>% 
        do.call(rbind, .), 
      lapply(kES$es_ebk, ReadDbf, intrpl = "RBF") %>% 
        do.call(rbind, .)
    )
  ) %>% 
...
lu.data %>% 
  ...
  mutate(es_ebk = factor(es_ebk, levels = kES$es_ebk)) %>% 
  ggplot() + 
  geom_bar(aes(landuse, val / 10^6, fill = es_ebk), 
